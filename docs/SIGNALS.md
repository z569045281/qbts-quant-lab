...
**推送覆盖时段**(worst-case 延迟):

| 时段(ET) | cron | 延迟 |
|---|---|---|
| 周日 20:00–23:59 | `SundayNightBtc` 每 10 分 | ≤10 分钟 |
| 周一–周五 00:00–03:59 | `OvernightMorning` 每分钟 | ≤1 分钟 |
| 周一–周五 04:00–19:59 | `MarketHoursEveryMinute` 每分钟 | ≤1 分钟 |
| 周一–周四 20:00–23:59 | `OvernightEvening` 每分钟 | ≤1 分钟 |

判定**不拉任何行情**(吃 quote 已算好的 `change_pct`),所以不挑分钟槽位、
每分钟都判,靠 `push_key`(日期+原因条数)每日去重。实测频率 ≈ **每月 4.7 次**。
...
- **冷却**:升 breaking 立推 / 同级别不同故事 45min / **降级不推**(公司消息"没新消息"
  不值得响铃,与地缘缓和不同)。
- **③要因不要果(2026-07-29 补,代码层)**:「Why ... Stock Surged Today」「Should You
  Buy the Stock Now?」这类描述价格结果的标题一律不推。这条规则提示词里**早就写了**,
  但 Haiku 照样把它们判成 high —— 按 2026-07-22 的教训,**规则必须落在代码里**
  (`is_price_result()` 正则)。
- **④无故事身份 = 只可能是转述(同上)**:剔掉无区分度的词后专名集合为**空**的标题
  (如「Why D-Wave Quantum Stock Surged Today」→ `set()`),连自己在讲什么都说不出来,
  不可能是一件新催化剂。
- **词干归一(同上)**:`_UBIQUITOUS` 枚举屈折形是填不完的坑 —— 表里有 `surge/surges`
  却没有 **`surged`**,于是那条转述稿的故事身份变成 `{surged}`,与 `{at&t,...}` 零交集,
  同一件事推了两遍。改为比对前过 `_stem()`。

> **实战复盘(2026-07-29)**:用户一天收到 6 条推送,其中 4 条讲的是同一笔 AT&T 交易。
> 修复后同一批标题回放 → **3 条**(AT&T 原始稿 / QUBT / IonQ 收购,确实是三件不同的事),
> 4 条价格结果稿全部拦下。
...
## ✂️ 摘除名单(2026-07-31 噪音审计,用户点单)

代码在 `edge.py` 文件头三张字典(`_REDUNDANT` / `_DEAD_SOURCE` / `_EVENT_DAY_MUTED`),
执行点 `edge._prune()`;`decision.py` 的经典策略段读**同一份名单**,防止元模型摘了、
LLM 还在算的"换个地方假共振"。

**判据必须是结构性的,不许按胜率砍。** 全部源当前 n=1~8,按命中率挑谁死正是
`audit.py` 预注册要防的多重比较错误。三条判据:

| 名单 | 判据 | 在册项 |
|---|---|---|
| `_REDUNDANT` | 同一事实被两个源读了两遍 → log-odds 相加的独立性前提不成立 | `Quantum Peer Lead/Lag`(与 `相对强度` 同吃 IONQ/RGTI 横截面;第十八轮已判死推广)· `Short Flow (Informed Shorts)`(与 `空头动向` 同一 FINRA 空量比) |
| `_DEAD_SOURCE` | 数据出处已不可核,不是"信号弱" | `散户情绪`(Reddit API 2026-06-05 起审批制+禁 AI 用途,Adanos 二手代理;07-30 全网仅 38 条提及撑 ±0.12 权重) |
| `_EVENT_DAY_MUTED` | 输入就是当日那根 bar,而系统自己在第二十八轮预注册过跳空≥8% 档 n=37 t=+0.36 **p=0.72 无分辨力** | `Post-News Overreaction` · `Gap-and-Trap` · `盘中量能`(**只在事件日**摘;期权/机制/13F 这些跨日证据不动) |

**实测影响**(2026-07-30 快照,+11.06% 事件日):元模型 `SELL p_up=37.6%` →
`HOLD p_up=43.6%`。翻转的一半来自 `Post-News Overreaction −0.22` —— 系统一边告诉
LLM「这天技术读数没有发言权」,一边让元模型拿同一根 bar 算出一个 SELL 递给 LLM 当
"交叉验证"。**这是系统跟自己打架,不是新判断。**

三条配套约定:
1. **v1 影子轨不过滤**(`compute_edge_v1` 吃原始清单)—— 它是冻结的历史重建,
   改了就没法跟 v2 对照。
2. **摘掉的必须看得见**:`edge["pruned"]` 带 `source/rule/why/would_have_been`,
   并写进决策 prompt。静默少算一个源跟静默多算一个源一样危险。
3. **陈旧 13F 不进 prompt**:报告期 >120 天时只写一行"今日不提供读数 + 不得引用"
   (与第二十轮判 SEC FTD「出版滞后 ≥2 周 = 结构性不可交易」同一条理由)。
   `holdings.py` 早就把方向推力静音了,但整段仍照写 200 多字 —— 静音一半等于没静音。

---

## 🔔 ntfy 推送优先级三档(2026-08-22,用户点单)

**起因**:`ntfy_log` 建账 4 天后第一次能数账 —— 31 条推送里 **16 条(52%)是地缘雷达**,
唯一一条真说「可以买」的(决策线触发)只有 1 条,还落在**墨尔本凌晨 6 点**
(全部 31 条里 15 条 = 48% 在墨尔本 00:00-08:00,因为美股收盘 = 墨尔本早 6 点)。
病根:**所有推送共用同一个 priority**,决策线触发和第 16 条伊朗新闻在手机上长得一样。

**分档依据不是感觉,是 8/15 审判的成绩单**(常量在 `notify.py`):

| 档 | 值 | 谁 | 依据 |
|---|---|---|---|
| `P_ACTION` | `urgent` | 决策线触发 · 事件日 · 特调 BUY/TRIM · 同行追赶 · SMC 15m 扳机 · 游击战开枪 · 财报落地 | 要你**动手**的东西,漏掉代价最大 |
| `P_NEWS` | `high` | 催化剂雷达 🔴 breaking | 值得知道,不必立刻动 |
| `P_AMBIENT` | `low` | **地缘雷达(全部)** · 催化剂 🟡 有消息 · 日检 OK | 地缘雷达 ②📋 记分卡 **n=2 命中 0% 技巧 −53pp**,第三十一轮定性「后视镜」——**没有实证分辨力的东西不该在半夜把人吵醒** |

**实测效果**(拿那 31 条重算):会真正响铃的从 **100% → 19%**。

### 地缘翻转熔断(`_FLIP_MAX = 2` / 24h)

08-13 加主题级冷却时给「升级」留了豁免口,理由是「补丁不许吃掉最该收到的那条」。
那个理由当时对,但台账数出真账:**四天 16 条,平均一天 4 条,冷却没拦住**。
病根就是那个豁免口 —— 伊朗局势在 🟡↔🔴 之间来回翻,**每次向上翻都算「升级」→ 每次都豁免**;
实测 08-19 一天之内翻了四次,四次全推。

**一件事一天翻四次,那不是升级,是抖动。** 熔断:24h 内翻转 > 2 次 → 判抖动,
当次升级**不再豁免**主题冷却。卡片 / prompt / `alerted` 登记照常,只关掉响铃。
⚠️ `flip_log` 必须跨 `unknown` 分支携带 —— 漏带 = 计数清零 = 抖动重新畅通(同一个坑第三次)。
