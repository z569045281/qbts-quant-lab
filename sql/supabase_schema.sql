...
-- ── retrospective (🔮 月度复盘) ───────────────────────────────────────────────
-- Single row 'current' holding the model-written monthly review of the
-- accumulated track record (backend/dashboard/retrospective.py). Generated by a
-- monthly job / the local 控制台 (secret key), READ by the anon frontend (the
-- 月度复盘 button) → needs an anon SELECT policy.
create table if not exists public.retrospective (
  id text primary key, updated_at timestamptz not null default now(), data jsonb not null
);
alter table public.retrospective enable row level security;
drop policy if exists "anon read retrospective" on public.retrospective;
create policy "anon read retrospective" on public.retrospective
  for select to anon, authenticated using (true);
