...
B. 一级信号（回测显著,优先采信）：
   ...
   - 特调双腿(用户自创,第十轮验证):「抄底建仓」(快%R上穿-80且慢<-50)——
     ⚠️ 别再写「全系统最强进场」:那个「后5天+17.4%」是 2024-07→2026-07(量子泡沫期)
     n=18 的**均值**;第三十四轮拉到全历史 n=55 = 均值+5.0%/**中位 −0.3%/胜率 44%**,
     姐妹 0/2 同向。它是**弱正腿**,可当加分项,不可当单独进场理由。
     且红灯下负期望(5日新低口径:绿灯后5天+3.7% vs 红灯 −1.0%,n=219/183)。
     「止盈减仓」(快%R下穿-20且慢≥-20)后20天 −10%,真能标顶(这条姐妹 3/3,可信)。
...
正好是 4**,最高从来没超过 6 —— 0-10 的量表塌成了一个点,而 4 恰好是「必须 HOLD」
门槛下面那一格。**一个从不变化的数字不携带任何信息**,它让「≤4→HOLD」这道闸门
变成了无条件生效。下面给每一档写死可核对的条件,**目的是让这个数字重新有区分度**。

⚠️ **这不是让你调高信心,是让你区分。** 证据弱时**必须往下走**(1-3 档现在几乎没被
用过,那同样是刻度坏了的表现);证据强时才往上。**如果照下表核对完仍然是 4,就给 4**
—— 诚实的 4 是好答案,条件反射的 4 不是。

  1-2  数据本身有问题(关键板块缺数/互相矛盾/报价存疑),今天不具备判断条件
  3    证据净方向为负 —— 一级信号里有反向的,且没有同级证据抵消
  4    **没有优势**:一级信号全部中性/沉默,或唯一表态的是二三级信号
  5    **一个**一级信号明确表态,且**没有**一级信号反向(规则2:单一信号封顶 5)
  6    **两个互相独立**的一级信号同向(独立 = 不共用同一个输入。QTUM 与 QBTS
       自身价格不算独立;板块 ETF 与 BTC 情绪算独立)
  7-8  ≥3 个一级信号共振,或 2 个一级信号同向 + 大盘红绿灯不逆风(规则2 门槛)
  9-10 上述条件 + 明确的、数据里能确认的催化剂;**至今从未出现过,别轻易用**

  ⚠️ 规则 8 的宏观上限(48h 内有 CPI/PPI/FOMC → 封顶 6)**优先于本表**。
  ⚠️ 本表只定义「这个数字是什么意思」,**不改变**任何仓位/闸门/铁律。
...
def _build_user_msg(snapshot: dict, extras: dict | None = None) -> str:
    ...
    if smc and smc.get("trend"):
        ...
        parts.append(
            ...
            f"  结构趋势: {smc['trend']}（方向锁用它）  {le_s}\n"
            # 2026-08-06 用户点单撤回 LuxAlgo,回到灵敏版结构引擎。它**翻多早、翻空
            # 慢**(翻空需收盘跌破未破的摆动低点,而反弹会抬高近期低点),所以
            # `trend=bullish` 更多是「小级别可能已转多」的早期提示,不是「大势已多」。
            "  ⚠️ 结构引擎口径: 破掉任一未被破的历史摆动高点即翻多 —— 早期信号,"
            "灵敏度换的是误报率;且它翻多快、翻回空慢，别拿 bearish 消失当转空凭据。\n"
