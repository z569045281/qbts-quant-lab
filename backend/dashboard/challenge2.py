...
第一期($1000→+$100,本地 bot,2026-07 达标 +10.7%)的升级复刻:
  资金   $5000 · 赢线 +10%($5500)· 地板 −15%($4250)· 期限 30 天
...
_START_CAP = 5000.0
_WIN_PCT = 0.10          # 赢线 = 本金 +10%
_FLOOR_PCT = 0.15        # 地板 = 本金 −15%
...
def _seed(sb, old: "dict | None", now_et: datetime) -> dict:
    ...
    st = {
        ...
        "deadline": (now_et + timedelta(days=_DAYS)).date().isoformat(),
    ...
    _log(st, f"🎬 第二期开局:本金 ${_START_CAP:,.0f},赢线 ${st['win_line']:,.0f},"
             f"地板 ${st['floor_line']:,.0f},截止 {st['deadline']}(云端 Lambda 全自动)")
    _save(sb, st)
    _ntfy("Challenge round 2 started",
          f"第二期开跑:$5,000 → +$500,截止 {st['deadline']}\n"
...
# ── 交易动作 ─────────────────────────────────────────────────────────────────
def _reconcile_close(st: dict, sym: str, fallback_px: "float | None") -> None:
    ...
    _log(st, f"CLOSED {sym} proceeds={approx}${proceeds:,.2f} pnl=${pnl:+,.2f}; "
             f"sleeve_cash=${st['sleeve_cash']:,.2f}")
...
def _liquidate(st: dict, sym: str, why: str) -> None:
...
def maybe_challenge_tick(now_et: datetime, force: bool = False) -> "dict | None":
    ...
    if sym:
        ...
        if pos is None:                       # bracket 在两跳之间自己成交了
            _reconcile_close(st, sym, _latest_px(sym))
            sym, pos = None, None
    ...
    if pos is not None:
        ...
        if unreal / entry_val >= _TP_TOUCH:
            _liquidate(st, sym, f"触碰 +{_TP_TOUCH*100:.0f}% 落袋")
        elif st["equity"] >= st["win_line"]:
            _liquidate(st, sym, f"权益触赢线 ${st['win_line']:,.0f}")
        elif st["equity"] <= st["floor_line"]:
            _liquidate(st, sym, f"权益触地板 ${st['floor_line']:,.0f}")

    # ── 空仓:到期/判定/进场 ──
    if st.get("position") is None:
        st["equity"] = st["sleeve_cash"]
        if st["equity"] >= st["win_line"]:
            _finish(sb, st, "won", "CHALLENGE WON.",
                    f"🏆 第二期达标!${st['equity']:,.2f}(+{(st['equity']/_START_CAP-1)*100:.1f}%)",
                    "trophy")
            return {"status": "won"}
