...
/** 紧凑模式共用的按钮外观 */
const COMPACT_BTN = "px-2.5 py-1 text-body font-medium rounded-inner bg-brand text-on-solid "
  ...
  if (compact) {
    return (
      <span className="inline-flex items-center gap-2">
        ...
        {result && (
          <span className={`text-meta ${result.ok ? "text-emerald-600" : "text-down"}`}>
  ...
  return (
    <Shell>
      <button
        onClick={run}
        disabled={busy}
        className="px-3.5 py-2 text-card font-medium rounded-inner bg-brand text-on-solid hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
      >
        {busy
      ...
      <span className="px-3 py-2 text-card rounded-inner border border-emerald-200 bg-emerald-50 text-emerald-700 flex items-center gap-2">
        <span className="inline-block w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
        实时报价 · 云端自动运行（盘前盘后）
      </span>

      {result && (
        <span className={`text-body ${result.ok ? "text-emerald-600" : "text-down"}`}>
          {result.ok ? `✓ ${result.msg}` : `✗ ${result.msg}`}
        </span>
      )}

      <p className="basis-full mt-2 text-meta text-[#9A9CA5]">
    ...
    return (
      <span className="inline-flex items-center gap-2">
        ...
        {!pub.running && pub.ok !== null && (
          <span className={`text-meta ${pub.ok ? "text-emerald-600" : "text-down"}`}>
  ...
  return (
    <Shell>
      ...
      <button
        ...
        className={`px-3.5 py-2 text-card font-medium rounded-inner border flex items-center gap-2 disabled:opacity-50 ${
          pusherOn
      ...
      <button
        onClick={runRetro}
        disabled={busy === "retro"}
        className="px-3.5 py-2 text-card font-medium rounded-inner border border-indigo-200 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 disabled:opacity-50 flex items-center gap-2"
      >
        {busy === "retro"
      ...
      {!pub.running && pub.ok !== null && (
        <span className={`text-body ${pub.ok ? "text-emerald-600" : "text-down"}`}>
          {pub.ok ? `✓ 决策已更新 ${pub.finished_at ?? ""}` : "✗ 出决策失败（见下方日志）"}
        </span>
      )}

      {retroMsg && (
        <span className={`text-body ${retroMsg.startsWith("✓") ? "text-emerald-600" : "text-down"}`}>
          {retroMsg}
        </span>
      )}

      {!pub.running && pub.ok === false && pub.log && (
        <pre className="basis-full mt-3 text-meta font-mono text-ink-muted bg-red-50 border border-red-100 rounded-inner px-3 py-2 max-h-40 overflow-auto whitespace-pre-wrap">{pub.log}</pre>
      )}
...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <section className="bg-surface rounded-card border border-hairline shadow-sm px-5 py-4">
      <div className="flex items-center gap-3 flex-wrap">
        <span className="text-section font-semibold text-gray-900 mr-1">控制台</span>
        {children}
      </div>
    </section>
  );
