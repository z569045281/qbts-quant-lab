  ...
  return (
    <section className="bg-surface rounded-card border border-hairline p-5 shadow-sm space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="ml-auto text-meta text-ink-faint">数据存在本机浏览器</span>
      </div>

      {/* 输入本次金额 */}
      <div className="flex items-end gap-2 flex-wrap">
        <label className="flex flex-col gap-1">
          <span className="text-meta text-ink-faint">本次要定投多少钱($)</span>
          <input
      ...
      {amt > 0 && split.length > 0 && (
        <div className="rounded-inner border border-black/5 bg-gray-50/60 p-3">
          <div className="text-meta text-ink-faint mb-1.5">这笔 {usd(amt)} 按建议权重应该这样买:</div>
            ...
            {split.map(s => (
              <div key={s.t} className="bg-surface rounded-inner px-2.5 py-2 border border-black/5">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-meta text-ink-faint">{s.w}%</span>
                </div>
                <div className="font-mono font-semibold text-brand text-card">{usd(s.money)}</div>
                {s.shares != null && (
                  <div className="text-meta text-ink-faint font-mono">≈{s.shares.toFixed(2)} 股 @${s.price!.toFixed(0)}</div>
                )}
              </div>
            ))}
          </div>
      ...
      <div className="rounded-inner bg-gradient-to-br from-blue-50 to-emerald-50 border border-blue-100 p-4">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div>
            <div className="text-meta text-ink-faint">累计已投入({list.length} 笔)</div>
            <div className="text-price font-bold text-gray-900 font-mono">{usd(totalInvested)}</div>
          </div>
          <label className="flex items-center gap-1.5 text-meta text-ink-faint">
            假设年化
            <input
            ...
            {horizons.map(y => {
              ...
              return (
                <div key={y} className="bg-surface/80 rounded-inner px-2 py-2.5 text-center">
                  <div className="text-meta text-ink-faint">{y} 年后</div>
                  <div className="text-section font-bold text-emerald-600 font-mono leading-tight">{usd(v)}</div>
                  <div className="text-meta text-ink-faint font-mono">×{(v / totalInvested).toFixed(1)}</div>
                </div>
              );
            })}
          </div>
        ) : (
          <p className="mt-2 text-meta text-ink-faint">记一笔、并填个年化%,这里就会显示复利后的样子。</p>
        )}
        <p className="mt-2.5 text-meta text-ink-faint leading-relaxed">
      ...
      {list.length > 0 && (
        <details className="text-body">
          <summary className="cursor-pointer text-ink-faint select-none">定投记录({list.length} 笔) ▾</summary>
