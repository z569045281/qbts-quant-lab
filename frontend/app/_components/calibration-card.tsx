    ...
    <section className="bg-surface rounded-inner border border-hairline overflow-hidden">
      ...
      <div className="px-5 py-4 grid grid-cols-1 lg:grid-cols-[200px_1fr] gap-5">
        ...
        <div className="space-y-2.5">
          <div>
            <div className="text-meta text-ink-faint">累计预测</div>
            <div className="text-price font-bold text-gray-900 font-mono">{cal.n_total}</div>
            <div className="text-meta text-ink-faint">{cal.n_graded} 条已可评判</div>
          </div>
          <div>
            <div className="text-meta text-ink-faint">整体方向命中</div>
            <div className={`text-price font-bold font-mono ${hitColor(cal.overall_hit_rate)}`}>
          ...
          {cal.n_total < 5 && (
            <div className="text-meta text-ink-faint italic leading-snug pt-2 border-t border-hairline">
        ...
        <div>
          <div className="text-meta text-ink-faint mb-2">
            各信号源（按样本量排序）
          </div>
          {bySorted.length === 0 ? (
            ...
            <div className="space-y-1">
              {bySorted.map(([src, info]) => (
                <div key={src} className="grid grid-cols-[1fr_60px_70px_70px] gap-2 text-body items-center">
                  <span className="truncate text-gray-900 font-medium" title={src}>{src}</span>
                  <span className="font-mono text-meta text-ink-faint text-right">
                    {info.hits}/{info.n}
                  </span>
                  <span className={`font-mono text-body font-semibold text-right ${hitColor(info.hit_rate)}`}>
                    {(info.hit_rate * 100).toFixed(0)}%
                  </span>
                  <span className={`font-mono text-meta text-right ${
          ...
          <p className="text-meta text-ink-faint mt-3 leading-snug">
