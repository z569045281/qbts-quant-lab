            ...
            return (
              <tr key={k} className={`border-t border-hairline ${mine ? "bg-brand/5" : ""}`}>
                <td className="py-1.5 pr-3 font-mono text-gray-800">
                  {k}
                  {mine && <span className="ml-1 text-[9px] text-brand">你的持有期</span>}
                </td>
...
function RecordRow({ r }: { r: SecondRecord }) {
  ...
  return (
    <div className="border-b border-hairline last:border-0 py-2.5">
          ...
          {["1d", "2d", "3d", "5d"].map(k => {
            ...
            return (
              <span key={k}
                    ...
                    className={`text-[10px] tabular-nums font-medium ${ok ? "text-emerald-700" : "text-down"}`}>
                {k}{ok ? "✓" : "✗"}
              </span>
            );
          })}
      ...
      <p className="mt-1 text-[11px] text-ink-muted leading-snug">{r.why}</p>
    </div>
  );
}
...
export function Board({ b }: { b: SecondBoard }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {/* 成绩单 */}
      <section className="bg-surface rounded-3xl shadow-[0_1px_2px_rgba(0,0,0,0.05)] p-5">
        <div className="flex items-baseline justify-between gap-2 flex-wrap mb-2">
          <span className="text-xs font-semibold text-ink-muted uppercase tracking-wider">
        ...
        <HorizonTable b={b} />
        <p className="mt-3 text-[10px] text-gray-400 bg-sunken rounded-lg px-2.5 py-2 leading-relaxed">
      ...
      {/* 为什么是这只票 + 自己推翻的那条 */}
      <section className="bg-surface rounded-3xl shadow-[0_1px_2px_rgba(0,0,0,0.05)] p-5 space-y-3">
        <div>
          <span className="text-xs font-semibold text-ink-muted uppercase tracking-wider">
            为什么选 {b.ticker}
          </span>
          <p className="mt-1.5 text-[12px] text-ink-muted leading-relaxed">{b.why_this_ticker}</p>
        </div>
