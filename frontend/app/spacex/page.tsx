    ...
    <div className="flex items-center gap-2">
      <div className="flex gap-0.5">
        {Array.from({ length: 10 }).map((_, i) => (
          <span key={i} className={`h-3 w-1.5 rounded-inner ${i < v ? "bg-brand" : "bg-gray-200"}`} />
        ))}
      </div>
      <span className="text-body font-mono font-semibold text-gray-600">{v}/10</span>
    </div>
  );
}
...
function DecisionHero({ d }: { d: SpacexDecision }) {
  ...
  return (
    <section className={`rounded-card border ${a.border} ${a.bg} p-5 shadow-sm`}>
      ...
      <p className="mt-3 text-card font-semibold text-gray-900 leading-relaxed">{d.summary}</p>
      ...
      {/* lockup note — 单独强调 */}
      {d.lockup_note && (
        <div className="mt-3 rounded-inner bg-surface/70 border border-amber-200 px-3 py-2">
          <div className="text-meta font-semibold text-amber-700">🔓 对 8/6 解禁的判断</div>
          <p className="text-body text-gray-600 mt-0.5 leading-relaxed">{d.lockup_note}</p>
        </div>
      )}
...
export default function SpacexPage() {
  ...
  return (
    <main className="max-w-[900px] mx-auto px-4 sm:px-6 py-5 sm:py-6 space-y-4">
      ...
      {/* 标题 */}
      <section className="rounded-card border border-hairline bg-gradient-to-br from-slate-900 to-slate-800 p-5 shadow-sm text-white">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-section">🚀</span>
          <span className="text-section font-bold">SpaceX · SPCX</span>
          <span className="text-meta px-2 py-0.5 rounded-full bg-white/15 font-mono">DeepSeek 决策</span>
          {dd && (
            <span className="ml-auto flex items-baseline gap-2">
              <span className="text-section font-mono font-bold">{usd(dd.price)}</span>
              <span className={`text-card font-semibold ${up ? "text-emerald-400" : "text-rose-400"}`}>{pct(dd.today_change)}</span>
            </span>
          )}
        </div>
        <p className="mt-1.5 text-meta text-slate-300/80 leading-relaxed">
        ...
        {WATCH_EDITABLE && (
          <div className="mt-3 flex items-center gap-2 flex-wrap">
            <button
              ...
              className="px-3.5 py-1.5 rounded-inner bg-brand text-on-solid text-body font-semibold
            ...
            {msg && <span className="text-meta text-slate-300/90 font-mono">{msg}</span>}
          </div>
        )}
      </section>
        ...
        <>
          ...
          {state.catalysts?.length > 0 && (
            <section className="rounded-card border border-amber-300 bg-amber-50 p-4 shadow-sm">
              <div className="text-body font-bold text-amber-800 mb-2">📅 事件日历(压倒机械技术位)· as of {state.catalyst_asof}</div>
              <div className="space-y-2">
                {state.catalysts.map((c, i) => (
                  <div key={i} className="flex gap-2 text-body">
                    <span className={`shrink-0 px-1.5 py-0.5 rounded-inner text-meta font-bold h-fit ${
          ...
          <section className="rounded-card border border-gray-300 bg-surface p-4 shadow-sm">
            <div className="text-body font-bold text-gray-800 mb-2">
            ...
            {!sc || sc.n_total === 0 ? (
              ...
              <>
                ...
                <div className={`text-meta rounded-inner px-2.5 py-1.5 mb-3 ${
          ...
          {state.options?.term && state.options.term.length > 0 && (
            <section className="rounded-card border border-violet-200 bg-violet-50/50 p-4 shadow-sm">
              <div className="text-body font-bold text-violet-800 mb-1">🔮 ① 期权隐含波动 · 市场对未来的预测(零历史需求)</div>
              <p className="text-meta text-violet-700/80 mb-3 leading-relaxed">
              ...
              {state.options.iv_degraded && (
                <div className="mb-2 rounded-inner border border-amber-300 bg-amber-50 px-2.5 py-1.5 text-meta text-amber-800">
                ...
                {state.options.term.map((x, i) => {
                  ...
                  return (
                    <div key={i} className={`flex items-center gap-2 text-body rounded-inner px-2.5 py-1.5 ${
                      ...
                      {isEvent && <span className="ml-auto text-meta font-bold text-rose-600">← 覆盖事件</span>}
                    </div>
                  );
                })}
              </div>
              <div className="mt-2 space-y-1 text-meta text-gray-600">
          ...
          {/* ② 盘中 1h(可靠技术面)*/}
          {state.intraday && (
            <section className="rounded-card border border-sky-200 bg-sky-50/50 p-4 shadow-sm">
              <div className="text-body font-bold text-sky-800 mb-1">
              ...
              {dd?.rsi14 != null && state.intraday.rsi14 != null && (
                <p className="text-meta text-sky-700/80 mb-3 leading-relaxed">
          ...
          {state.news?.length > 0 && (
            <section className="rounded-card border border-hairline bg-surface p-4 shadow-sm">
              <div className="text-body font-semibold text-gray-700 mb-2">📰 近3日头条(喂给 DeepSeek 的原料)</div>
